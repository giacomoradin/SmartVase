...
int main() {
    ...
    // --- hubEpochPlausible(epoch_s) — Hub->Mega time sync (proto v4.2) ---
    CHECK(!hubEpochPlausible(0),            "epoch 0 (Hub without NTP) -> ignored");
    CHECK(!hubEpochPlausible(28800),        "epoch from a fake 1970 clock -> ignored");
    CHECK(!hubEpochPlausible(1599999999UL), "epoch below the 2020 threshold -> ignored");
    CHECK(hubEpochPlausible(1600000000UL),  "epoch at the threshold -> accepted");
    CHECK(hubEpochPlausible(1780000000UL),  "present-day epoch (2026) -> accepted");
